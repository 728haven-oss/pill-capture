...
set -euo pipefail
# Capacitor 7 은 JDK 21 필요 → 러너에 내장된 JDK 21 사용 (이후 스텝에도 적용)
if [ -n "${JAVA_HOME_21_X64:-}" ]; then export JAVA_HOME="$JAVA_HOME_21_X64"; echo "JAVA_HOME=$JAVA_HOME_21_X64" >> "${GITHUB_ENV:-/dev/null}"; echo "$JAVA_HOME_21_X64/bin" >> "${GITHUB_PATH:-/dev/null}"; fi
java -version 2>&1 | head -1
npm install
npx cap add android
npx cap sync android
# Play 요구: targetSdk 36 이상 (2026) → SDK 36 + AGP 8.9 / Gradle 8.11 (Capacitor 7 + RevenueCat 11 = Play Billing 8)
...
# 카메라 권한: <input type=file capture> 가 WebView에서 카메라를 열 수 있도록 (Capacitor가 런타임 권한 요청 처리)
